...
func TestParseClingyFile(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "test expected failure on 00_will_badly_fail.yaml",
			args: args{
				logger:   log.Default(),
				fileName: "../cmd/test_data/00_will_badly_fail.yaml",
			},
			wantErr:        true,
			wantErrPartial: "unable to process template, no steps",
		},
		{
			name: "test pass case on 01_basic_flow_will_pass.yaml",
			args: args{
				logger:   log.Default(),
				fileName: "../cmd/test_data/01_basic_flow_will_pass.yaml",
			},
			wantErr:        false,
			wantErrPartial: "",
		},
